...
** CSS and JavaScript aren't byte code !! :rant:css:javascript:web:
   ...
   :CUSTOM_ID: css-and-javascript-aren-t-byte-code
   :END:
...
** Setting up a celery task scheduler in Flask         :python:flask:celery:
   :PROPERTIES:
...
** GatsbyJs: An overkill for a static site generator :gatsby:react:emacs:hugo:orgmode:rant:editor:
   ...
   :CUSTOM_ID: gatsbyjs-an-overkill-for-a-static-site-generator
   :END:
...
** A minimal tmux configuration from scratch :tmux:productivity:unix:mac:
   :PROPERTIES:
...
** Using Auth0 with a custom user store :auth:python:
   ...
   :EXPORT_HUGO_DRAFT: true
   :CUSTOM_ID: using-auth0-with-a-custom-user-store
   :END:
...
:PROPERTIES:
:CUSTOM_ID: auth0-actions-configuration
:END:
